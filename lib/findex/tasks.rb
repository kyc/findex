  ...
  def get_model_reflection_indices(model, indices, existing_indices)
    ...
    model.reflections.each do |name, reflection|
      ...
      when :belongs_to
        ...
        indices[model.table_name].push(foreign_key) if check_index(foreign_key, indices[model.table_name], existing_indices) && model.column_names.include?(reflection.primary_key_name)
